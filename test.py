...
class SvgTestCase(unittest.TestCase):
    ...
    def test_svg_piece_set_embeds_source_asset(self):
        piece = chess.Piece.from_symbol("P")
        piece_svg = chess.svg.piece(piece, piece_set="cburnett")
        root = chess.svg.ET.fromstring(piece_svg)
        image = root.find("{http://www.w3.org/2000/svg}image")
        self.assertIsNotNone(image)
        encoded = image.get("href").partition(",")[2]
        embedded = base64.b64decode(encoded).decode("utf-8")
        self.assertEqual(embedded, chess.svg.load_pieces("cburnett")["wP"])

    def test_svg_piece_set_adds_origin_viewbox(self):
        image = chess.svg._embedded_piece('<svg width="90" height="45"><path /></svg>')
        encoded = image.get("href").partition(",")[2]
        embedded = chess.svg.ET.fromstring(base64.b64decode(encoded))
        self.assertEqual(embedded.get("viewBox"), "0 0 90 45")

    def test_svg_board_piece_set(self):
